...
const FR = require(path.join(__dirname, '..', 'content-bank-fr.js'));
...
const packFr = ctx.window.CONTENT_FR || ctx.CONTENT_FR;
const excl = {
  ...
  fr: packFr.lessons.flatMap(l => l.vocab.map(v => v.en))
};
...
function coreWord(w, lang) {
  ...
  if (lang === 'fr') s = s.replace(/^(le|la|les|un|une|des)\s+/, '').replace(/^l'/, '');
  return s;
}
...
checkBank(FR, 'content-bank-fr.js', 'fr', excl.fr);
